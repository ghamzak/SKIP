...
def valid_direction(relation: str, direction: str, rel_rules: Dict[str, str]) -> bool:
    """
    Check the grammatical role of the entity (subj or obj) is valid or not, according to the rel_rules
    """
...
def select_triple(entity: str, raw_triples: List[str], context_set: Set[str],
                  rel_rules: Dict[str, str], max: int) -> Tuple(List[str], int, int):
    """
    ...
    for line in raw_triples:
        ...
        # if the grammatical role of the entity (subj or obj) does not match, skip this
        ...
        if direction == 'LEFT': # entity concept is subject in cpnet triple, so take the object stem as neighbor
            neighbor = set(triple[4].strip().split('_'))
        elif direction == 'RIGHT': # entity concept is object in cpnet triple, so take the subject stem as neighbor
            neighbor = set(triple[1].strip().split('_'))
        
        if in_context(concept = neighbor, context = stem_context): # check if the neighbor occurs in the context (topic + paragraph)
    ...
    triples_by_relevance = [t for t in triples_by_relevance if get_weight(t) >= 1.0] # since triples_by_relevance contains line (str) not triple (list), use get_weight and get_relation functions over them
    ...
    triples_by_score = list(set(triples_by_score) - set(triples_by_relevance)) # all that were not in the triples_by_relevance
...
if __name__ == "__main__":
    ...
    parser.add_argument('-input', type=str, default='./rough_retrieval.json', help='path to the rough conceptnet triples')
