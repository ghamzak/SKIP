...
koala_dir = '/home/ghazaleh/dissertation/neural_symbolic/LEXIS3/KOALA/'
# model parameters
parser.add_argument('-per_gpu_batch_size', type=int, default=18)
...
parser.add_argument('-ckpt_dir', type=str, default='checkpoints', help="directory to save checkpoints and logs")
...
parser.add_argument('-impatience', type=int, default=35,
                    help='number of evaluation rounds for early stopping, use -1 to disable early stopping')
parser.add_argument('-report', type=int, default=2, help="report frequency per epoch, should be at least 1")
parser.add_argument('-train_set', type=str, default=koala_dir+"data/train.json", help="path to training set")
parser.add_argument('-dev_set', type=str, default=koala_dir+"data/dev.json", help="path to dev set")
parser.add_argument('-no_cuda', action='store_true', default=False, help="if true, will only use cpu")

# test parameters
parser.add_argument('-test_set', type=str, default=koala_dir+"data/test.json", help="path to test set")
parser.add_argument('-restore', type=str, default=None, help="path to saved checkpoint")
parser.add_argument('-dummy_test', type=str, default=koala_dir+"data/dummy-predictions.tsv", help="path to prediction file template")
parser.add_argument('-output', type=str, default='/data/ghazaleh/neuralsymbolic/predict/prediction.tsv', help="path to store prediction outputs")

# commonsense parameters
parser.add_argument('-cpnet_path', type=str, default=koala_dir+"ConceptNet/result/retrieval.json",
...
parser.add_argument('-state_verb', type=str, default=koala_dir+'ConceptNet/result/state_verb_cut.json',
...
try:
    ...
    # opt.n_gpu = 0 if opt.no_cuda else torch.cuda.device_count()
    # print('Number of available GPUs:', torch.cuda.device_count(), ' --> ', os.environ["CUDA_VISIBLE_DEVICES"])
...
def train():
    ...
    while epoch_i < opt.epoch:
        ...
        for batch in train_batch:
            # print('starting training on batch', mybatch_idx)            
            paragraphs = batch['paragraph']
            # print(f'len(paragraphs), which should be batch_size:{len(paragraphs)}, type(paragraphs):{type(paragraphs)}')
            # print('paragraph batched')
            # print('paragraphs from batch in train file: \n', paragraphs)
            token_ids = plm_tokenizer.batch_encode_plus(paragraphs, add_special_tokens=True, padding=True,
                                                        return_tensors='pt')['input_ids']
            # print(f'token_ids size:{token_ids.size()}')
            # print('token_ids of the above paragraph from train file:\n', token_ids)
            # print('paragraphs tokenized, and the size is', token_ids.size())
            ...
            # print('cpnet_triples:', cpnet_triples)
            # print('cpnet_triples from train file:', cpnet_triples)
            # cpnet_triples_altered = []
            # for i in cpnet_triples:
            #     new_internal = [x for x in i if x]
            #     if new_internal:
            #         cpnet_triples_altered.append(new_internal)
            # cpnet_triples = cpnet_triples_altered
            ...
            # print('num_cands is a', type(num_cands))

            if not opt.no_cuda:
                token_ids = token_ids.cuda()
                # print('token_ids copied to GPU')
                sentence_mask = sentence_mask.cuda()
                # print('sentence_mask copied to GPU')
                entity_mask = entity_mask.cuda()
                # print('entity_mask copied to GPU')
                verb_mask = verb_mask.cuda()
                # print('verb_mask copied to GPU')
                loc_mask = loc_mask.cuda()
                # print('loc_mask copied to GPU')
                gold_loc_seq = gold_loc_seq.cuda()
                # print('gold_loc_seq copied to GPU')
                gold_state_seq = gold_state_seq.cuda()
                # print('gold_state_seq copied to GPU')
                state_rel_labels = state_rel_labels.cuda()
                # print('state_rel_labels copied to GPU')
                loc_rel_labels = loc_rel_labels.cuda()
                # print('loc_rel_labels copied to GPU')
                num_cands = num_cands.cuda()
                # print('num_cands copied to GPU')
            ...
            # print(f'checking token_ids once more:{token_ids.size()}')
            ...
            # print('trained model for batch', mybatch_idx)
            mybatch_idx += 1
...
def evaluate(dev_set, model, tb_writer, report_cnt: int):
    ...
    with torch.no_grad():
        for batch in dev_batch:
            ...
            # if max([len(token_ids[i]) for i in range(len(token_ids))]) == 0:
            #     print('paragraphs:',paragraphs)
            sentence_mask = batch['sentence_mask']
            entity_mask = batch['entity_mask']
            verb_mask = batch['verb_mask']
